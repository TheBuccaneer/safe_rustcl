...
fn main() -> Result<(), ClError> {
    ...
    println!("Testing BASIC RAW bandwidth:");
    ...
    for _ in 0..num_buffers {
        gpu_buffers.push(Buffer::<f32>::create(&context, CL_MEM_READ_WRITE, chunk_size, ptr::null_mut())?);
    }
    ...
    for iter in 0..iterations {
        ...
        // Alle Chunks parallel
        for (i, gpu_buf) in gpu_buffers.iter_mut().enumerate() {
        ...
        if iter == 0 {
            println!("First H2D (basic raw): {:.3} ms", elapsed * 1000.0);
        }
    }
    ...
    for iter in 0..iterations {
        ...
        for (i, gpu_buf) in gpu_buffers.iter().enumerate() {
            let start_idx = i * chunk_size;
            let end_idx = start_idx + chunk_size;
        ...
        if iter == 0 {
            println!("First D2H (basic raw): {:.3} ms", elapsed * 1000.0);
        }
    }
    ...
    // 8) Results - gleiche Ausgabe wie Wrapper
    ...
    let pcie3_x16 = 12.8;
    println!("\n=== RAW EFFICIENCY ===");
    println!("Pure raw efficiency: H2D {:.1}%, D2H {:.1}%", 
