...
impl CopyToken {
    ...
    fn log_once(&mut self) {
        ...
        let abort = CURRENT_ABORT.lock().unwrap().clone(); //  fehlte
    ...
    abort_token: abort,
    // NEU:
    phase: match self.dir { Dir::Kernel => Phase::Kernel, _ => Phase::Transfer },
    tx_id: None,
    cause: None,
    retries: None,
    conflict_sz: None,
...
pub fn log_transfer(t_start_us: u64, t_end_us: u64, bytes: usize, dir: Dir) {
    ...
    log.push(Record {
        t_start_us, t_end_us, bytes, dir, idle_us: idle, abort_token: abort,
        phase: if matches!(dir, Dir::Kernel) { Phase::Kernel } else { Phase::Transfer },
        tx_id: None, cause: None, retries: None, conflict_sz: None,
    });
}